...
模块缺失时，结构化界面仍允许查看、搜索、调整布局，修改基座拥有的 `label` 和 `meta`，修改关系的 `source`、`target` 和 `direction`，以及使用正常的可恢复机制删除对象或关系。涉及关系重连的保存结果只能声称通过基础校验，不能声称符合缺失模块的领域规则。

结构化 Web UI、CLI 和 MCP 不得修改未知模块拥有的 `data`、增删未知 `capabilities`，也不得直接把未知 `kind` 改为另一类型。改变主类型必须由能够理解源类型和目标类型的显式转换或模块迁移完成。

YAML 是事实来源，用户始终可以直接编辑原始文件。基座读取这种修改后执行基础校验，保留所有未知内容，并明确标记完整校验不可用。

### 无损往返场景

以缺少 `research` 模块、但仍安装 `exploration` 模块为例：

1. 基座读取 `research.question` 对象，同时保留未知 `data` 和已知、未知能力状态；
2. 用户通过通用界面修改对象 `label`，或调整与它相连的关系；
3. 基座保存时只重写已获授权的基座字段，未知模块拥有的数据保持相同语义结构；
4. 基础校验检查格式、ID 唯一性和端点完整性，并报告缺少 `research`，因此完整校验不可用；
5. 安装兼容当前 `schema` 的 `research` 模块后，原 `data` 恢复模块表单、规则和操作。
